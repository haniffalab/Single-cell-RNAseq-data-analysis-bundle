...
output_folder = paste("12_plot_dr", seurat.addr, sep = "_")
c.time = Sys.time()
...
for (index in 1:length(plot.by)){
  ...
  plot.fdg_unlabeled    = dr.plot(point.labels=df$CellType, dr1=df$FDGx, dr2=df$FDGy, dr1.name="FDG-x", dr2.name="FDG-y", no.legend=T, plt.lb.sz=7, txt.lb.size=4, use.cols=cell.colours, limits=fdg.limits, use.labels=cell.labels, annotate.plot = F)
