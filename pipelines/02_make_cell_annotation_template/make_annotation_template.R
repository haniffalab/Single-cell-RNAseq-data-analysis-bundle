...
tabulate.by1.arg   = args[3]
tabulate.by2.arg   = args[4]
DE.downsample.arg  = args[5]
"
...
seurat.obj@meta.data$LouvainClustering = paste("Cluster", as.vector(seurat.obj@meta.data$LouvainClustering), sep = "_")
...
if (DE.downsample){
  ...
  print("Finished subseting, currently actually calculating the markers ... ")
  marker.genes = FindAllMarkers(object = seurat.obj_d, only.pos = F, min.pct = 0.1, 
                                genes.use = rownames(seurat.obj@data), test.use = "wilcox",
                                random.seed = 42, print.bar=T, do.print=T, max.cells.per.ident = 300)
}else{
  marker.genes = FindAllMarkers(object = seurat.obj, only.pos = F, min.pct = 0.1, 
                                genes.use = rownames(seurat.obj@data), test.use = "wilcox",
                                random.seed = 42, print.bar=T, do.print=T, max.cells.per.ident = 500)
}
...
df = data.frame(CellNames = names(seurat.obj@ident),
                 ...
                 Sample = seurat.obj@meta.data$fetal.ids)
...
tabulate.seurat.by.cluster(seurat.obj, tabulate.by1, tabulate.by2, save.at="./graphs", width=1110, height=110, saveas.pdf = F)

# compile template annotation powerpoint
system(paste(python.addr, "compile_template.py", sep = " "), wait = T)

unlink("graphs", recursive=T, force=T)
file.remove(file.path(output_folder, 'compile_template.py'))
